...
let error_folder = "error_metadata";
...
async function getTokenMetadata(token_address: string) {
  try {
    ...
    fs.writeFileSync(
      metadata_folder + "/" + metadataFileName,
    ...
    fs.writeFileSync(
      airweave_metadata_folder + "/" + arweaveFileName,
      JSON.stringify(arweave)
    );
    ...
    console.log(e);
    const errorFileName = `${token_address}.json`;
    fs.writeFileSync(
      error_folder + "/" + errorFileName,
      JSON.stringify(token_address)
    );
  }
}

// getTokenMetadata(nft_address)

//  Read the json file nft_mint_addresses.json
...
const save_metadata = async () => {
  for (const nft of nfts) {
    console.log(nft);
    // Check if the nft is already saved in the metadata folder by checking if the is a file name partially that contains the nft address in the name
    if (fs.readdirSync(metadata_folder).some((file) => file.includes(nft))) {
